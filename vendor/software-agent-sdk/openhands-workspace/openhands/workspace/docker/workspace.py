...
class DockerWorkspace(RemoteWorkspace):
    ...
    _preserve_container: bool = PrivateAttr(default=False)
        ...
        while time.time() - start < timeout:
            ...
            if self._container_id:
                ...
                if ps.stdout.strip() != "true":
                    ...
                    if os.getenv("OPENHANDS_DOCKER_KEEP_FAILED_CONTAINERS") in {
                        "1",
                        "true",
                        "TRUE",
                    }:
                        self._preserve_container = True
                        msg += f"\nPreserved failed container: {self._container_id}"
    ...
    def cleanup(self) -> None:
        ...
        if self._container_id:
            ...
            if self._preserve_container:
                logger.warning(
                    "Preserving failed container for debugging: %s",
                    self._container_id,
                )
